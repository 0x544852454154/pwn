...
export default function EditChallengePage() {
  ...
  const fetchChallenge = useCallback(async () => {
    if (!id) return;
    try {
      const response = await fetch(`/api/challenges/${id}/manage`);
      ...
      setForm({
        ...
        points: c.points,
        estimatedTime: c.estimated_time || 30,
        flag: c.flag,
        visibility: c.visibility
      });
      setObjectives(c.objectives?.length > 0 ? c.objectives.map(o => o.objective) : ['']);
      setHints(c.hints?.length > 0 ? c.hints.map(h => ({ text: h.hint_text, penalty: h.point_penalty })) : [{ text: '', penalty: 0 }]);
      setError('');
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const response = await fetch(`/api/challenges/${id}/manage`, { method: 'DELETE' });
      const data = await response.json();
  ...
  return (
    <>
      <Head>
        <title>pwnlab · edit challenge</title>
      </Head>

      <Layout requireAuth={true}>
        <div className={styles.container}>
          <div className={styles.header}>
            <div className={styles.kicker}>challenge workshop / edit</div>
